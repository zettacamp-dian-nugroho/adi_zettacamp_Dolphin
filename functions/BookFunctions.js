...
const InsertOneBookData = async (BookData) => {
  ...
  return await Book.save()
    ...
    .catch((err) => {
      console.log(`Function Error ${err.name}`);
      return `Gagal insert data! ${err.name}`;
    });
};
...
const InsertBookDatas = async (BookData) => {
  ...
  for (const listbookdatas of BookData) {
    const Book = new Books({
      book_title: listbookdatas.title,
      author: listbookdatas.author,
      price_of_book: listbookdatas.price,
      stock_of_book: listbookdatas.stock,
    });
...
const UpdateOneBookData = async (BookId, BookData) => {
  ...
  if (isHaveBookDatas) {
    return await Books.findByIdAndUpdate(tempId, UpdateDataBook, { new: true })
      .then((result) => {
        return `Berhasil update data buku!`;
      })
      .catch((err) => {
        return `Gagal update data! ${err.name}`;
      });
  } else {
    return `Gagal update data! Data buku tidak ada.`;
  }
};

const UpdateAuthorBook = async (author, BookData) => {
  const tempAuthor = { author: author.split(':')[1] };
  const isHaveBookDatas = await Books.find(tempAuthor);

  for (const listbook of isHaveBookDatas) {
    const UpdateDataBook = {
      book_title: listbook.book_title,
      author: listbook.author,
      price_of_book: listbook.price_of_book,
      stock_of_book: listbook.stock_of_book,
    };

    if (isHaveBookDatas) {
      return await Books.updateMany(tempAuthor, UpdateDataBook, { new: true })
        .then((result) => {
          return `Berhasil update data author!`;
        })
        .catch((err) => {
          return `Gagal update data! ${err.name}`;
        });
    } else {
      return `Gagal update data! Data author tidak ada.`;
    }
  }
};
...
const DeleteOneBookData = async (BookId) => {
  ...
  if (isHaveBookDatas) {
    return await Books.findByIdAndDelete(tempId)
      .then((result) => {
        return `Berhasil menghapus data buku!`;
      })
      .catch((err) => {
        return `Gagal menghapus data! ${err.name}`;
      });
  } else {
    return `Gagal menghapus data! Data buku tidak ada.`;
  }
};
