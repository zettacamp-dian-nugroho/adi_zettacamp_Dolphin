...
const GetOneBookDataWithProject = async (AuthorName) => {
  const isHaveBookDatas = await Books.aggregate([
    {
      $match: {
        author: AuthorName,
      },
    ...
    {
      $addFields: {
        isCanBeSell: { $gt: ['$stock_of_book', 0] },
        howManyGenres: { $size: '$book_genre' },
      },
    },
  ]);
...
const GetOneBookByPublisher = async (PublisherName) => {
  const isHaveBookDatas = await Books.aggregate([
    {
      $match: {
    ...
    {
      $addFields: {
        book_title_with_author: { $concat: ['$book_to_title', ' - ', '$author'] },
      },
    },
  ]);
...
const GetBooksDataPagination = async (Page, Limit) => {
  const isHaveBookDatas = await Books.aggregate([
    {
      $unwind: '$book_genre',
    },
    ...
    {
      $addFields: { on_page: parseInt(Page) },
    },
  ]);
...
const GetBooksDataPaginationFacet = async (Page, Limit) => {
  const isHaveBookDatas = await Books.aggregate([
    {
      $unwind: '$book_genre',
    },
...
module.exports = {
  ...
  GetBookDatas,
  GetOneBookData,
  GetOneBookDataWithProject,
  GetOneBookByPublisher,
  GetBooksDataPagination,
  GetBooksDataPaginationFacet,
  UpdateOneBookData,
  DeleteOneBookData,
  ResetBookCollection,
};
