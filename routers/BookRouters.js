...
router.get('/get-books/pagination/:page/:limit/', async (req, res) => {
  try {
    const getOneBook = await Book.GetBooksDataPagination(req.params.page, req.params.limit);
    if (!getOneBook) {
      res.json({ messages: 'Data tidak ada!' });
    } else {
      res.json({ messages: getOneBook });
...
router.get('/get-books/pagination/facet/:page/:limit/', async (req, res) => {
  try {
    const getOneBook = await Book.GetBooksDataPaginationFacet(req.params.page, req.params.limit);
    if (!getOneBook) {
      res.json({ messages: 'Data tidak ada!' });
    } else {
      res.json({ messages: getOneBook });
